...
Three brutal, real-world prompts. Every one is solvable with the existing `pagila` schema and data — no extra tables required. Query the partitioned `payment` parent table directly (it transparently spans `payment_p2022_01 … _07`).
...
**Requirements.**
1. Each rental occupies the half-open interval `[rental_date, return_date)`. If `return_date IS NULL`, treat the film as still out through the **latest timestamp present in the data** (max of all rental/return dates).
2. Map rentals to a store via `inventory.store_id` (`rental → inventory`).
3. Build an **event stream**: `+1` at each `rental_date`, `-1` at each `return_date`. Compute a **running sum ordered by event time** to get concurrent-out count, partitioned per store.
...
**Why it's hard.** This is the classic interval-overlap / sweep-line problem: you must `UNION ALL` two derived event sets with opposite deltas, order carefully (a return at the exact instant of a rental must net correctly), run a windowed cumulative sum, then pick the argmax per partition with a tie-break on earliest time.
...
2. For every (cohort_month, activity_month) pair, count distinct paying customers, and compute the **month offset** (0 = cohort month, 1 = next month, …).
3. Compute **retention %** = active customers at offset N / cohort size at offset 0.
4. Present it as a **retention matrix**: one row per cohort, columns for offsets 0 through 6 (the data spans Jan–Jul 2022). Use conditional aggregation / `FILTER` to pivot.
...
## Challenge 3 — Overdue Rental & Late-Fee Liability Analysis
...
**Requirements.**
1. A rental's **due date** = `rental_date + film.rental_duration` days (join `rental → inventory → film`).
2. Classify each rental:
   - **On time** — returned on or before due date.
   - **Late** — returned after due date (`days_late` = whole days past due).
...
3. Model a **late fee** = `days_late × (film.replacement_cost / film.rental_duration)`, i.e. a daily rate proportional to the film's value — but **cap** the total fee at `film.replacement_cost`.
4. Aggregate **per store**: number of late rentals, number of outstanding rentals, average days late (late + outstanding combined), and total capped late-fee liability.
5. Also surface the single **worst offending customer per store** (highest accumulated late fee) using a window ranking — return their name alongside the store totals.
